...
CREATE TYPE job_type AS ENUM (
  'full_time', 'part_time', 'freelance', 'internship', 'temporary'
);
...
CREATE TYPE guide_category AS ENUM (
  'nie', 'empadronamiento', 'autonomos', 'seguridad_social',
  'bancos', 'vivienda', 'educacion', 'sanidad', 'otro'
);

CREATE TYPE ad_position AS ENUM (
  'home_hero', 'sidebar', 'footer', 'jobs_top', 'companies_top'
);

-- ============================================================
-- 2. FUNCIÓN update_updated_at (compartida)
-- ============================================================
...
CREATE POLICY "companies_read_active"   ON companies FOR SELECT USING (is_active AND is_approved);
CREATE POLICY "companies_owner_read"    ON companies FOR SELECT USING (auth.uid() = owner_id);
CREATE POLICY "companies_owner_insert"  ON companies FOR INSERT WITH CHECK (auth.uid() = owner_id);
CREATE POLICY "companies_owner_update"  ON companies FOR UPDATE USING (auth.uid() = owner_id) WITH CHECK (auth.uid() = owner_id);
CREATE POLICY "companies_owner_delete"  ON companies FOR DELETE USING (auth.uid() = owner_id);
...
CREATE POLICY "jobs_read_active"    ON jobs FOR SELECT USING (is_active AND is_approved AND expires_at > NOW());
CREATE POLICY "jobs_owner_read"     ON jobs FOR SELECT USING (auth.uid() = posted_by);
CREATE POLICY "jobs_owner_insert"   ON jobs FOR INSERT WITH CHECK (auth.uid() = posted_by);
CREATE POLICY "jobs_owner_update"   ON jobs FOR UPDATE USING (auth.uid() = posted_by) WITH CHECK (auth.uid() = posted_by);
CREATE POLICY "jobs_owner_delete"   ON jobs FOR DELETE USING (auth.uid() = posted_by);
...
CREATE POLICY "events_read_active"       ON events FOR SELECT USING (is_active AND is_approved AND date_start > NOW() - INTERVAL '1 day');
CREATE POLICY "events_organizer_read"    ON events FOR SELECT USING (auth.uid() = organizer_id);
CREATE POLICY "events_organizer_insert"  ON events FOR INSERT WITH CHECK (auth.uid() = organizer_id);
CREATE POLICY "events_organizer_update"  ON events FOR UPDATE USING (auth.uid() = organizer_id) WITH CHECK (auth.uid() = organizer_id);
CREATE POLICY "events_organizer_delete"  ON events FOR DELETE USING (auth.uid() = organizer_id);
...
CREATE POLICY "guides_author_read"    ON guides FOR SELECT USING (auth.uid() = author_id);
CREATE POLICY "guides_admin_all"      ON guides FOR ALL
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'admin'));
...
CREATE POLICY "ads_read_active" ON advertisements FOR SELECT
  USING (is_active AND starts_at <= NOW() AND ends_at >= NOW());
CREATE POLICY "ads_admin_all"   ON advertisements FOR ALL
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'admin'));
