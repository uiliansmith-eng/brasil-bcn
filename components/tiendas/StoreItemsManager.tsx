...
import { Plus, Pencil, Tag, ChevronDown, ChevronUp } from 'lucide-react'
import { StoreItemForm } from './StoreItemForm'
import { StoreItemVariantsManager } from './StoreItemVariantsManager'
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6 sm:p-8">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div key={item.id} className="p-4 rounded-xl border border-gray-100">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    ...
                    {item.track_stock && ` · ${item.stock ?? 0} uds`}
                  </p>
                </div>
              ...
              {expandedId === item.id && <StoreItemVariantsManager storeItemId={item.id} />}
            </div>
          ))}
        </div>
      )}
