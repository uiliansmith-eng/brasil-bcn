...
export default async function MiTiendaPage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <Link
        href={hasMultipleStores ? '/dashboard/tienda' : '/dashboard'}
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link
            href={`/dashboard/tienda/${company.id}/reservas`}
            className="flex items-center gap-1.5 text-sm font-semibold text-gray-600 hover:text-gray-900"
          >
            <CalendarClock className="w-3.5 h-3.5" /> Reservas
          </Link>
          <Link
            href={`/dashboard/tienda/${company.id}/resenas`}
            className="flex items-center gap-1.5 text-sm font-semibold text-gray-600 hover:text-gray-900"
          >
      ...
      {hasMultipleStores && (
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-400 hover:text-gray-600 mb-4"
        >
          <Store className="w-3.5 h-3.5" /> Cambiar de tienda
        </Link>
      )}
