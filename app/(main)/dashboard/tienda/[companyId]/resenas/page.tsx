...
import { getCompanyForStaff } from '@/actions/stores'
...
export default async function TiendaResenasPage({ params }: PageProps) {
  ...
  const reviews = await getStoreReviewsForOwner(access.company.id) as (Review & { user: { full_name: string | null } | null })[]

  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <Link href={`/dashboard/tienda/${access.company.id}`} className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-700 text-sm mb-6 transition-colors">
